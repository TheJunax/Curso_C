...
int leerEncabezado(FILE *archivo, unsigned int *cantidadEstudiantes,unsigned int *cantidadCursos, unsigned int *cantidadMaterias){
    ...
    leidos = fread(cantidadMaterias, sizeof(unsigned int), 1, archivo);
    if(leidos != 1){
...
int main(int argc, char *argv[]){
    ...
    unsigned int cantidadEstudiantes, cantidadCursos, cantidadMaterias;

    if(leerEncabezado(archivo, &cantidadEstudiantes, &cantidadCursos, &cantidadMaterias) != 0){
        fclose(archivo);
        return 1;
    }
