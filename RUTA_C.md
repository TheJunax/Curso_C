...
- **Funciones completadas:** `son_amigos` y `obtener_amigos` (helpers), `es_reflexiva` (cada persona p tiene loop (p,p) vía `son_amigos`), `es_simetrica`, `encontrar_clique_de_tamano` (ternas i<j<k con los 3 pares amigos), `contar_amigos_mutuos` (intersección de listas de `obtener_amigos`), `puede_alcanzar` (BFS con cola + visitado), `todos_grados_pares` (grado vía `obtener_amigos`, `%2`).
- **⚠️ Decisión final de `es_simetrica` (versión del estudiante, sin cambios):** código híbrido — recorre cada arista buscando su inversa explícita, pero en el `if(encontrado == 0)` hace `return 1` (conceptualmente invertido: debería ser `return 0`). Como todo termina en `return 1`, **siempre dice "Sí"** → coincide con la salida esperada del quiz (amistad mutua/no dirigida), pero el bloque de "detección" no decide nada. Se documentó al estudiante; quedó pendiente decidir si se limpia o se justifica como "amistad mutua por definición".
- **Salida esperada exacta (requisito del estudiante):** "Sí (0,1,2)" para el clique y "1 (persona 2)" para los amigos mutuos, **sin cambiar las firmas originales** del enunciado → se logró con impresión directa dentro de las funciones (*side effect* de `printf`; en software serio se devuelven por parámetros de salida, pero la firma del quiz no lo permite). La salida coincide 100% con los 2 casos de prueba del enunciado.
...
- **Verificación:** `gcc -Wall -Wextra` con salida 100% idéntica a la esperada en ambos casos (Caso 1: No/Sí/Sí (0,1,2)/1 (persona 2)/Sí/Sí; Caso 2: No/Sí/No/1 (persona 1)/No/No). **Warning residual:** `n` sin usar en `puede_alcanzar` (así estaba el código del estudiante; se puede silenciar con `(void) n`).
