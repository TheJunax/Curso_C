...
int inicializarInventario(Producto **inventario, int *cantidadProductos, int *capacidad) {
    *cantidadProductos =0;
    *capacidad = 3;
    *inventario = (struct*)malloc((*capacidad) * sizeof(struct)); //Ya despues supe que era con Producto
    if(*inventario == NULL){
...
int agregarProducto(Producto **inventario, int *cantidadProductos, int *capacidad, int id, char nombre[], int cantidad, float precio) {
    int nuevaCapacidad = (*capacidad) * 2, *tempInventario = NULL;
    if(*cantidadProductos == *capacidad){
      tempInventario = (struct*)realloc(*inventario, nuevaCapacidad* sizeof(struct));
    ...
    inventario[(*cantidadProductos)-1]->id = id;
    inventario[(*cantidadProductos)-1]->nombre = nombre;  //Tenia la idea pero me enrrede con el **  y tambien recorde despues que era con el strcpy
    inventario[(*cantidadProductos)-1]->cantidad = cantidad;
    inventario[(*cantidadProductos)-1]->precio = precio; 
...
void actualizarCantidad(Producto *inventario, int cantidadProductos, int id, int nuevaCantidad) {
    ...
    else{printf("Producto con ID %d no encontrado\n", id);
    }
    }
    /*Esta función debe:
* Buscar un producto por su id.
